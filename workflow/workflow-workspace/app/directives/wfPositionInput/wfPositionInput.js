angular.module('wfDirectives').directive('wfPositionInput', ['$mdDialog', '$filter', '$timeout', '$http', 'CONFIG',
	/**
	 ...
	 */
	function ($mdDialog, $filter, $timeout, $http, config) {
		return {
			require: 'ngModel',
			restrict: 'E',
			scope: {
			...
			link: function (scope, element, attributes, controller) {
				...
				window.setTimeout(initMap.bind(this), 100);
				//initMap();

				/**
				 * @memberOf wfPositionInput
				 * @desc Center to given position
				 * 
				 * @param {event} event
				...
				/**
				 * @memberOf wfPositionInput
				 * @desc Checks if given post code equals with the municipality's one
				 * 
				 * @param {Number} location
				 * @param {Boolean} isNew
				 */
				checkPostCode = function (location, isNew) {
