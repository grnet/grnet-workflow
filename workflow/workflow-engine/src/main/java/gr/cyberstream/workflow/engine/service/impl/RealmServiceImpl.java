...
public class RealmServiceImpl implements RealmService {
	...
	public WfUser getUser(String id) {
		...
		try {
			...
			if (response != null && response.getBody() != null) {
				...
				user.setGroups(response.getBody().getGroups());
	...
	public List<String> getRealmGroups() {
		...
		try {
			...
			URI uri = KeycloakUriBuilder.fromUri(keycloakServer + "/admin/realms/{realm}/groups").build(keycloakRealm);

			ResponseEntity<GroupRepresentation[]> response = restTemplate.exchange(uri, HttpMethod.GET, entity,
					GroupRepresentation[].class);
	...
	private List<String> getUserRoles(RestTemplate restTemplate, HttpHeaders headers, String userID) {
		...
		try {
			...
			URI uri = KeycloakUriBuilder
					.fromUri(keycloakServer + "/admin/realms/{realm}/users/{id}/role-mappings/realm/composite")
					.build(keycloakRealm, userID);

			ResponseEntity<RoleRepresentation[]> response = restTemplate.exchange(uri, HttpMethod.GET, entity,
			...
			logger.error(
					"GET '/admin/realms/{realm}/users/{id}/role-mappings/realm/composite' failed: " + e.getMessage());
		}
