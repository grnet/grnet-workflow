...
public class DefinitionController {
	 ...
	 * 
	 * Returns specific process definition by id
	 * including the start form definition
	 * 
	 * @return
	 * @throws InvalidRequestException
	 */
	...
	/**
	 * <code>GET: /api/v2/process/filter/{option};owner={...}|all</code>
	 *  
	 * Returns process definitions filtered by the owners
	 * 
	 * @return
	 * @throws InvalidRequestException 
	 */
	@RequestMapping(value = "/process/filter/{option}", method = RequestMethod.GET)
	@ResponseBody
	@PreAuthorize("hasAnyRole('ProcessAdmin','Admin')")
	public List<WfProcess> getProcessDefinitionsByOwner(@PathVariable("option") String option,
			@MatrixVariable(pathVar="option", required=false) MultiValueMap<String, String> filterValues) throws InvalidRequestException {

		if (option.equals("owners") && filterValues.containsKey("owner")) {
			
			if (filterValues.get("owner").contains("all")) {
				return processService.getAll();				
			} else {
				return processService.getDefinitionsByOwners(filterValues.get("owner"));
			}
			
		} else {
			throw new InvalidRequestException("Invalid request parameters. GET: /engine/api/v2/process/{option};{all|owner=...}");
		}
	}
	
	/**
	 * <code>POST: /api/v2/process/bpmn</code>
	 * 
	...
	public WfProcess createProcessDefinition(@RequestPart("file") MultipartFile uploadedFile)
		...
		try {
			...
			return processService.createNewProcessDefinition(uploadedFile.getInputStream(),
	 ...
	 * 
	 * @param process the process definition to be updated
	 * @param request
	...
	public Resource getProcessDiagram(@PathVariable int id) {
		...
		return processService.getProcessDiagram(id);
	}
	 ...
	 * 
	 * @param processId the process id
	 * @param versionId the version id to become active
	 * 
	 * @return
	 * @throws InvalidRequestException
	 */
	 ...
	 * 
	 * @param processId the process id
	 * @param versionId the version id to be deactivated
	 * 
	 * @return
	 * @throws InvalidRequestException
	 */
