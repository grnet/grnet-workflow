...
public class DefinitionControllerTest {
	...
	@Autowired
	DefinitionController definitionV2Controller;
	...
	public void setup() {
		...
		try {
			...
			if (!integration) {
				unwrappedController = (DefinitionController) unwrapProxy(definitionV2Controller);
	...
	public void getAllProcessDefinitions() throws Exception {
		List<WfProcess> processes = Arrays.asList(new WfProcess[] { new WfProcess() });
	...
	public void getProcessDefinition() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(get("/api/v2/process/{id}", processId).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void getProcessDefinitionForm() throws Exception {
		...
		when(processService.getProcessMetadata(any(Integer.class), any(String.class))).thenReturn(process);

		ResultActions resultActions = mockMvc
				.perform(
						get("/api/v2/process/{id}/form?client=" + client, processId).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void filterProcessDefinitions() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(get("/api/v2/process/filter/owners;owner=GroupA,GroupB").accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void filterAllProcessDefinitions() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(get("/api/v2/process/filter/owners;owner=all").accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void createProcessDefinition() throws Exception {
		...
		MockMultipartFile file = new MockMultipartFile("file", "DocumentTestProcess.bpmn", MediaType.TEXT_XML_VALUE,
				content);

		mockMvc.perform(fileUpload("/api/v2/process").file(file).contentType(MediaType.MULTIPART_FORM_DATA))
	...
	public void updateProcessDefinition() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(put("/api/v2/process").content("{}").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void deleteProcessDefinition() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(delete("/api/v2/process/{id}", id).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void createProcessVersion() throws Exception {
		...
		mockMvc.perform(fileUpload("/api/v2/process/{id}/version", processId).file(file)
	...
	public void activateProcessDefinitionVersion() throws Exception {
		...
		when(processService.setActiveVersion(any(Integer.class), any(Integer.class))).thenReturn(new WfProcess());

		ResultActions resultActions = mockMvc
				.perform(put("/api/v2/process/{processId}/version/{versionId}/active", processId, versionId)
	...
	public void deactivateProcessDefinitionVersion() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(put("/api/v2/process/{processId}/version/{versionId}/inactive", processId, versionId)
						.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void deleteProcessDefinitionVersion() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(delete("/api/v2/process/version/{processId}/{deploymentId}", processId, deploymentId)
						.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void updateSettings() throws Exception {
		...
		ResultActions resultActions = mockMvc
				.perform(put("/api/v2/settings").content("{}").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void claimFacebookToken() throws Exception {
		...
		mockMvc.perform(post("/api/v2/facebook").content("{}").contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	}
