...
public class ExecutionControllerTest {
    ...
    public void setup() {
		...
		try {
			...
			if (!integration) {
				unwrappedController = (ExecutionController) unwrapProxy(executionController);
	...
	public void getExecutions() throws Exception {
		...
		int id = 1;
		List<WfProcessInstance> executions = Arrays.asList(new WfProcessInstance[]{
				new WfProcessInstance()
		});
		
		when(processService.getActiveProcessInstances(any(Integer.class))).thenReturn(executions);
		
		ResultActions resultActions = mockMvc.perform(get("/api/v2/execution/process/version/{id}", id)
	...
	public void getSupervisedExecutions() throws Exception {
		...
		ResultActions resultActions = mockMvc.perform(get("/api/v2/execution/supervised").accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void getUserCompletedExecutions() throws Exception {
		...
		ResultActions resultActions = mockMvc.perform(get("/api/v2/execution/completed").accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void getExecutionById() throws Exception {
		...
		ResultActions resultActions = mockMvc.perform(get("/api/v2/mobile/execution/{id}", id).accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void startProcess() throws Exception {
		...
		ResultActions resultActions = mockMvc.perform(post("/api/v2/process/{id}/start", id)
	...
	public void startProcessWithDocuments() throws Exception {
		...
		mockMvc.perform(
				fileUpload("/api/v2/process/{id}/start", processId)
	...
	public void startPublicProcess() throws Exception {
		...
		when(processService.startPublicProcess(any(String.class), any(WfProcessInstance.class))).thenReturn(processInstance);
		
		ResultActions resultActions = mockMvc.perform(post("/api/v2/public/form/{id}/start", id)
				.content(instanceData).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void startPublicProcessWithDocuments() throws Exception {
		...
		when(processService.startPublicProcess(any(String.class), any(WfProcessInstance.class), any(MultipartFile[].class)))
			.thenReturn(new WfProcessInstance());
		...
		mockMvc.perform(
				fileUpload("/api/v2/public/form/{id}/start", id)
					.file(data).file(file).contentType(MediaType.MULTIPART_FORM_DATA))
					.andExpect(status().isOk())
					.andExpect(jsonPath("$.*").exists());
	}
	...
	public void startMobileProcess() throws Exception {
		...
		when(processService.startPublicMobileProcess(any(String.class), any(WfProcessInstance.class))).thenReturn(processInstance);
		
		ResultActions resultActions = mockMvc.perform(post("/api/v2/mobile/form/{id}/start", id)
				.content(instanceData).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void startMobileProcessWithDocuments() throws Exception {
		...
		when(processService.startPublicMobileProcess(any(String.class), any(WfProcessInstance.class), any(MultipartFile[].class)))
			.thenReturn(new WfProcessInstance());
		...
		mockMvc.perform(
				fileUpload("/api/v2/mobile/form/{id}/start", id)
					.file(data).file(file).contentType(MediaType.MULTIPART_FORM_DATA))
					.andExpect(status().isOk())
					.andExpect(jsonPath("$.*").exists());
	}
	...
	public void deleteExecution() throws Exception {
		...
		mockMvc.perform(delete("/api/v2/execution/{id}", id)
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	}
	...
	public void cancelExecution() throws Exception {
		...
		mockMvc.perform(put("/api/v2/execution/{id}/cancel", id)
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	}
	...
	public void changeExecutionStatus() throws Exception {
		...
		ResultActions resultActions = mockMvc.perform(put("/api/v2/execution/{id}/{action}", id, action)
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void getExecutionForm() throws Exception {
		...
		ResultActions resultActions = mockMvc.perform(get("/api/v2/execution/{id}/form", id)
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	...
	public void getProcessInstanceDocuments() throws Exception {
		...
		ResultActions resultActions = mockMvc.perform(get("/api/v2/execution/{id}/document", id)
				.accept(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
