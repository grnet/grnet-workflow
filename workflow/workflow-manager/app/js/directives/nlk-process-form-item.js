define(['angular'],
	...
	function (angular) {
			...
			function itemTemplate(type) {
				...
				switch (type) {
					...
					case 'document':
						inputTmpl =
							'<section layout="row" class="md-block">' +
								...
								'<wf-document-input id="{{ngModel.id}}" wf-variable-code="{{ngModel.id}}" flex="50" ' +
						...
						inputTmpl =
							'<section layout="row" class="md-block"> ' +
								...
								'<wf-position-input id="{{ngModel.id}}" ' +
									...
									'wf-position-center-lng="{{wfPositionCenterLng}}" wf-id-prefix="{{wfIdPrefix}}" ' +
					...
					case 'enum':
						inputTmpl =
							'<section layout="row" class="md-block">' +
							'<md-input-container class="md-block {{ngModel.required ? \'wf-required\' : \'\'}}" flex="25">' +
		...
		angular.module('wfManagerDirectives').directive('nlkProcessFormItem', ['$compile', '$mdToast', '$window', '$filter', 'CONFIG', processFormItem]);
	}
);