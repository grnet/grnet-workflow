define(['angular'],
	...
	function (angular) {
		function singleSeriesChart($compile) {
			return {
				require: 'ngModel', // the ng-model is required
				restrict: 'E', // only matches element name
				controller: 'wfSingleSeriesChartCtrl', // directive's controller
				...
				templateUrl: 'templates/singleSeries.tmpl.html',
		...
		function singleSeriesChartCtrl($scope, $compile, $element, dashboardService) {
			...
			var lineTmpl = '<canvas id="' + $scope.ngModel.id + '" class="chart chart-line" chart-options="lineOptions" chart-data="lineData" chart-labels="labels"></canvas>';
				...
				dashboardService.getData($scope.ngModel.src).then(
					function (response) {
						...
						for (var data in response.data) {
							...
							$scope.data.push(response.data[data].value);
						}

						$scope.lineData.push($scope.data);
		...
		angular.module('wfManagerDirectives').directive('wfSingleSeriesChart', ['$compile', singleSeriesChart]);

		// Directive's controller
		angular.module('wfManagerControllers').controller('wfSingleSeriesChartCtrl', ['$scope', '$compile', '$element', 'dashboardService', singleSeriesChartCtrl]);
	}
);